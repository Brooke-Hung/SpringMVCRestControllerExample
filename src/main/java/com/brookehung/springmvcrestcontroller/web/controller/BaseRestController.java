 ...
 */
@RequestMapping(value="/api/v1")//work with spring v.5.1 + 
public abstract class BaseRestController<T, PK, SC, ESC> implements RestController<T, PK, SC, ESC> {
	...
	public ResponseEntity<List<T>> search(SC sc) {
		ESC esc = convertSCToESC(sc);
		List<T> list = getService().search(esc);
	...
	protected ESC convertSCToESC(SC sc){
		throw new UnsupportedOperationException();
	}
	
	protected abstract Service<T, ESC, PK> getService();
